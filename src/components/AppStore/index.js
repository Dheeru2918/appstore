...
class AppStore extends Component {
  state = {value: tabsList[0].tabId, filters: ''}

  onChanged = eachItem => {
    this.setState({value: eachItem.tabId})
  }

  getFilter = tabId => {
    const data = appsList.filter(eachItem => eachItem.category === tabId)
    return data
  }
  ...
  getfull = data => {
    ...
    const ret = data.map(eachItem => eachItem.appName.includes(filters))
    return ret
  }

  render() {
    const {value} = this.state
    const data = this.getFilter(value)
    const datas = this.getfull(data)
    return (
      <div>
        <h1>App Store</h1>
